...
If IIPA encounters any issues during the validation of the data shared by partners, IIPA will send out and the CO’s will have to communicate with the partners regarding the same.
After that, the CO can request IIPA for manually generating the missing partner’s file so that IIPA can fulfil it.
...
The name of the file should have the month in which the file is loaded and should not be basis the data inside. Therefore, for the month of June 2025, we should have 202506 as the file name even if the data we have in the file is until 202505.
...
All files to be provided in the
dataos-core-prod-iibi-vendor-in is the S3 bucket for inbound files. Inside this bucket, we have folders for all partners, and they upload their files to their respective folders.
...
Please reach out to parthiban.manavalan@hp.com and andrea.ferrer.cucurella@hp.com for any questions or issues with access.
